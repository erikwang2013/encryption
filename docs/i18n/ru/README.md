...
SM3 и SM4-CBC используют нативные реализации OpenSSL, если подключённый OpenSSL предоставляет `sm3` / `sm4-cbc` (OpenSSL 1.1.1+). В противном случае они переходят на чистые PHP-реализации из `pohoc/crypto-sm` — вывод побайтово идентичен, но работает гораздо медленнее: у запасного варианта SM3 квадратичный рост памяти (~490 MB и ~85 s на один дайджест в 1 MiB против ~4 MB при ~60 MB/s нативно). CI прогоняет набор тестов по обоим путям.
...
Фабрика мастер-ключа: `EncryptionManagerFactory::fromMasterKey($masterKey32, 'aes-256-gcm')` выводит подключи для каждого алгоритма и сразу регистрирует **aes-256-gcm**, **aes-256-cbc-hmac**, **sm4-cbc**, **zuc-128** и **sodium-xchacha20** (если доступен ext-sodium). По умолчанию вывод использует `'v1'`; передайте `'v2'` третьим аргументом, чтобы получить исправленный порядок аргументов HMAC (они взаимно нечитаемы — см. §8).
...
### 8. Схема вывода ключей v1 → v2 (необязательная миграция)

**Что было не так.** При выводе подключей для каждого алгоритма и MAC-ключей `aes-256-cbc-hmac`, `sm4-cbc` и `zuc-128` функция `hash_hmac($algo, $data, $key)` вызывалась с постоянной меткой назначения в роли **ключа** HMAC, а секретный материал — в роли **сообщения**: аргументы были перепутаны местами. Ключом HMAC должен быть секрет.

**Почему это не эксплуатируется.** Метка назначения — публичная константа, а секрет по-прежнему участвует в HMAC; атакующий без мастер-ключа (или ключа шифра) ничего не выведет и ничего не подделает. Неверным был только порядок аргументов, а не стойкость вывода.

**Как переключиться.** Передайте `'v2'` третьим аргументом. Если его опустить, сегодняшнее поведение сохраняется побайтово, поэтому все существующие места вызова остаются без изменений:
...
Каждый шифратор на основе MAC принимает тот же переключатель последним необязательным аргументом — `new Sm4CbcEncryptor($key16, macDerivation: 'v2')`. Неизвестное имя схемы выбрасывает `EncryptionException`, а не молча откатывается к v1.

**Миграция.** v1 и v2 выводят *разные* подключи и MAC-ключи, поэтому они не взаимозаменяемы: менеджер v2 не может расшифровать шифртекст v1, а менеджер v1 — шифртекст v2 (сбой MAC / аутентификации). Маркера схемы в передаваемых данных нет — обе схемы пишут один и тот же префикс полезной нагрузки `v1` — поэтому во время миграции чтение с неверной схемой неотличимо от подделанного шифртекста: и то и другое проявляется как сбой MAC. Во время миграции логируйте схему, использованную при каждом чтении, и при таких сбоях сначала предполагайте несовпадение схемы, а уже потом — повреждение. Создайте оба менеджера из одного мастер-ключа и работайте по принципу «сначала чтение, затем запись»:
...
Перешифруйте сохранённые данные (включая сессии и токены) и выведите менеджер v1 из эксплуатации, когда шифртекста v1 больше не останется. Сам мастер-ключ не меняется: смена схемы вывода — это не ротация ключа.

**Не связано с префиксом полезной нагрузки.** `v1` в структуре полезной нагрузки `v1 | IV | MAC | ciphertext` — это версия *формата шифртекста*, а не данной схемы вывода: под v2 префикс остаётся `v1`, а старые блобы сохраняют свой. Не переименовывайте его.
...
├── .github/workflows/tests.yml      phpunit на PHP 8.0–8.4 в CI (gmp + sodium)
...
5. **Нашли уязвимость?** Сообщите о ней приватно — см. [`SECURITY.md`](../../../SECURITY.md).
