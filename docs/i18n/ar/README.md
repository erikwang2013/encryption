...
يستخدم SM3 وSM4-CBC تطبيقات OpenSSL الأصلية متى وفّر OpenSSL المرتبط `sm3` / `sm4-cbc` (OpenSSL 1.1.1+). وإلا فإنهما يعودان إلى تطبيقات PHP الصرفة في `pohoc/crypto-sm` — الناتج متطابق بايتًا ببايت، لكنه أبطأ بكثير: فالمسار الاحتياطي لـ SM3 تربيعي في استهلاك الذاكرة (~490 MB و~85 s لبصمة واحدة بحجم 1 MiB، مقابل ~4 MB عند ~60 MB/s أصلًا). ويشغّل CI مجموعة الاختبارات على المسارين.
...
مصنع المفتاح الرئيسي: تشتق `EncryptionManagerFactory::fromMasterKey($masterKey32, 'aes-256-gcm')` مفاتيح فرعية لكل خوارزمية، وتسجّل **aes-256-gcm** و**aes-256-cbc-hmac** و**sm4-cbc** و**zuc-128** و**sodium-xchacha20** (إذا كانت ext-sodium متاحة) دفعة واحدة. يستخدم الاشتقاق `'v1'` افتراضيًا؛ ومرّر `'v2'` كوسيط ثالث للحصول على ترتيب وسائط HMAC المصحَّح (الاثنان غير قابلين للقراءة أحدهما بالآخر — انظر §8).
...
### 8. مخطط اشتقاق المفاتيح v1 → v2 (ترحيل اختياري)

**ما كان الخطأ.** عند اشتقاق المفاتيح الفرعية لكل خوارزمية ومفاتيح MAC الخاصة بـ `aes-256-cbc-hmac` و`sm4-cbc` و`zuc-128`، كان `hash_hmac($algo, $data, $key)` يُستدعى مع تمرير تسمية الاستخدام الثابتة كـ**مفتاح** لـ HMAC والمادة السرية كـ**رسالة** — أي أن الوسيطين كانا مقلوبين. ويجب أن يكون المفتاح في HMAC هو المادة السرية.

**لماذا لا يمكن استغلالها.** تسمية الاستخدام ثابت عام، والمادة السرية لا تزال تدخل في HMAC؛ والمهاجم الذي لا يملك المفتاح الرئيسي (أو مفتاح التشفير) لا يشتق شيئًا ولا يزوّر شيئًا. ولم يكن الخطأ إلا في ترتيب الوسائط، لا في قوة الاشتقاق.

**كيفية التبديل.** مرّر `'v2'` كوسيط ثالث. وإذا حذفته بقي السلوك الحالي كما هو بايتًا ببايت، فلا يتغير أي موضع استدعاء قائم:
...
تقبل كل أداة تشفير تعتمد على MAC المفتاح نفسه كوسيط اختياري أخير — `new Sm4CbcEncryptor($key16, macDerivation: 'v2')`. واسم المخطط غير المعروف يرمي `EncryptionException` بدلًا من العودة الصامتة إلى v1.

**الترحيل.** يشتق v1 وv2 مفاتيح فرعية ومفاتيح MAC *مختلفة*، لذا لا يمكن تبادلهما: فمدير v2 لا يستطيع فك تشفير نص v1، ومدير v1 لا يستطيع فك تشفير نص v2 (فشل MAC / المصادقة). ولا توجد علامة على المخطط في البيانات المنقولة — فكلاهما يكتب بادئة الحمولة `v1` ذاتها — لذلك يصعب أثناء الترحيل تمييز القراءة بالمخطط الخطأ عن نص مشفَّر متلاعب به: فكلاهما يظهر كفشل في MAC. سجّل المخطط المستخدم في كل قراءة أثناء الترحيل، وتوقّع أن تكون هذه الأخطاء ناتجة عن عدم توافق في المخطط قبل أن تشك في تلف البيانات. ابنِ المديرين من المفتاح الرئيسي نفسه واقرأ أولًا ثم اكتب:
...
أعد تشفير البيانات المخزّنة (بما فيها الجلسات والرموز) وأخرج مدير v1 من الخدمة متى لم يبقَ أي نص مشفَّر بـ v1. المفتاح الرئيسي نفسه لا يتغير: فتغيير مخطط الاشتقاق ليس تدويرًا للمفاتيح.

**لا علاقة له ببادئة الحمولة.** الـ `v1` في بنية الحمولة `v1 | IV | MAC | ciphertext` هو إصدار *صيغة النص المشفَّر*، وليس مخطط الاشتقاق هذا — فالبادئة تبقى `v1` في v2، وتحتفظ الكتل القديمة ببادئتها. لا تعد تسميته.
...
├── .github/workflows/tests.yml      تشغيل phpunit على PHP 8.0–8.4 في CI (gmp + sodium)
...
5. **وجدت ثغرة؟** أبلغ عنها بشكل خاص — راجع [`SECURITY.md`](../../../SECURITY.md).
