...
SM3 ও SM4-CBC লিংক করা OpenSSL `sm3` / `sm4-cbc` সরবরাহ করলে (OpenSSL 1.1.1+) OpenSSL-এর নেটিভ ইমপ্লিমেন্টেশন ব্যবহার করে। অন্যথায় তারা `pohoc/crypto-sm`-এর বিশুদ্ধ-PHP ইমপ্লিমেন্টেশনে ফিরে যায় — আউটপুট বাইটে-বাইটে অভিন্ন, তবে অনেক ধীর: SM3 ফলব্যাক মেমোরিতে দ্বিঘাত (একটি 1 MiB ডাইজেস্টে ~490 MB ও ~85 s, নেটিভে ~4 MB-তে ~60 MB/s)। CI দুটি পথেই স্যুট চালায়।
...
মাস্টার-কী ফ্যাক্টরি: `EncryptionManagerFactory::fromMasterKey($masterKey32, 'aes-256-gcm')` প্রতি অ্যালগরিদমের সাব-কী তৈরি করে এবং একবারেই **aes-256-gcm**, **aes-256-cbc-hmac**, **sm4-cbc**, **zuc-128** ও **sodium-xchacha20** (ext-sodium থাকলে) রেজিস্টার করে। ডিরাইভেশন ডিফল্টভাবে `'v1'` ব্যবহার করে; সংশোধিত HMAC আর্গুমেন্ট ক্রমের জন্য তৃতীয় আর্গুমেন্ট হিসেবে `'v2'` দিন (দুটি একে অপরকে পড়তে পারে না — §8 দেখুন)।
...
### 8. কী-ডিরাইভেশন স্কিম v1 → v2 (ঐচ্ছিক মাইগ্রেশন)

**কী ভুল ছিল।** প্রতি-অ্যালগরিদম সাব-কী এবং `aes-256-cbc-hmac`, `sm4-cbc` ও `zuc-128`-এর MAC কী ডিরাইভ করার সময় `hash_hmac($algo, $data, $key)`-কে ধ্রুবক ব্যবহার-লেবেল HMAC **কী** হিসেবে এবং গোপন উপাদান **বার্তা** হিসেবে দেওয়া হচ্ছিল — দুটি আর্গুমেন্ট পরস্পর বদলে গিয়েছিল। HMAC-এর কী অবশ্যই গোপন উপাদান হতে হবে।

**কেন এটি শোষণযোগ্য নয়।** ব্যবহার-লেবেল একটি প্রকাশ্য ধ্রুবক এবং গোপন উপাদান এখনও HMAC-এ যায়; মাস্টার কী (বা সাইফার কী) ছাড়া আক্রমণকারী কিছুই ডিরাইভ বা জাল করতে পারে না। কেবল আর্গুমেন্টের ক্রম ভুল ছিল, ডিরাইভেশনের শক্তি নয়।

**কীভাবে সুইচ করবেন।** তৃতীয় আর্গুমেন্ট হিসেবে `'v2'` দিন। এটি বাদ দিলে আজকের আচরণ বাইটে-বাইটে অপরিবর্তিত থাকে, তাই বিদ্যমান প্রতিটি কল সাইট অপরিবর্তিত থাকে:
...
MAC-ভিত্তিক প্রতিটি এনক্রিপ্টর শেষ ঐচ্ছিক আর্গুমেন্ট হিসেবে একই সুইচ নেয় — `new Sm4CbcEncryptor($key16, macDerivation: 'v2')`। অজানা স্কিম নাম নীরবে v1-এ ফিরে না গিয়ে `EncryptionException` ছোড়ে।

**মাইগ্রেশন।** v1 ও v2 *ভিন্ন* সাব-কী ও MAC কী ডিরাইভ করে, তাই তারা বিনিময়যোগ্য নয়: একটি v2 ম্যানেজার v1 সাইফারটেক্সট ডিক্রিপ্ট করতে পারে না, আর একটি v1 ম্যানেজার v2 সাইফারটেক্সট পারে না (MAC / প্রমাণীকরণ ব্যর্থতা)। স্কিমের কোনো অন-ওয়্যার চিহ্ন নেই — দুটোই একই `v1` পেলোড প্রিফিক্স লেখে — তাই মাইগ্রেশনের সময় ভুল স্কিমে পড়া টেম্পার করা সাইফারটেক্সট থেকে আলাদা করা যায় না: দুটোই MAC ব্যর্থতা হিসেবে দেখা দেয়। মাইগ্রেশনের সময় প্রতিটি পড়ায় ব্যবহৃত স্কিম লগ করুন, এবং এমন ব্যর্থতাকে ডেটা নষ্ট হওয়া ভাবার আগে স্কিমের অমিল হিসেবে ধরে নিন। দুটি ম্যানেজার একই মাস্টার কী থেকে তৈরি করুন এবং আগে পড়ুন, পরে লিখুন:
...
সংরক্ষিত ডেটা (সেশন ও টোকেনসহ) পুনরায় এনক্রিপ্ট করুন এবং v1 সাইফারটেক্সট আর না থাকলে v1 ম্যানেজার অবসর দিন। মাস্টার কী নিজে বদলায় না: ডিরাইভেশন স্কিম বদলানো কী রোটেশন নয়।

**পেলোড প্রিফিক্সের সাথে সম্পর্কিত নয়।** `v1 | IV | MAC | ciphertext` পেলোড কাঠামোর `v1` হলো *সাইফারটেক্সট ফরম্যাট* সংস্করণ, এই ডিরাইভেশন স্কিম নয় — v2-এও প্রিফিক্স `v1` থাকে, আর পুরনো ব্লব নিজের প্রিফিক্স রাখে। এটির নাম বদলাবেন না।
...
├── .github/workflows/tests.yml      CI-তে PHP 8.0–8.4-এ phpunit (gmp + sodium)
├── composer.json                    psr-4 অটোলোড, PHP ^8.0, phpunit ডেভ ডিপেন্ডেন্সি
├── phpunit.xml.dist
├── SECURITY.md                      দুর্বলতা প্রকাশ নীতি
...
5. **কোনো দুর্বলতা পেয়েছেন?** ব্যক্তিগতভাবে জানান — দেখুন [`SECURITY.md`](../../../SECURITY.md)।
